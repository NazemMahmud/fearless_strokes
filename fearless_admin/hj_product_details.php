...
//		 save_scaled($file1,$midimg,$ext,400,220);
      ...
      $img_insert=mysqli_query($con, "UPDATE hj_product_image SET 
					   BigImage ='".$bigimgname."' ,
                       MidImage ='".$midimgname."' ,
                       SmallImage = '".$smallimgname." ' 
                       WHERE ImageID='".$img_id."' AND ProductID='".$ProductID."'  ");
     
    // echo $bigimgname; echo $midimgname; echo $smallimgname;
 ...
 if(isset($_POST["btnDelete"]))
 {
    $id=$_POST["id"];
	 ...
	 $activity_update=mysqli_query($con, "UPDATE hj_product_info SET UpdateDate='".$Date."', UpdateBy='".$_SESSION['UserID']."'
		  WHERE ProductID='".$ProductID."'");
 ...
 if(isset($_POST["btnSave"]))
 {
    ...
    $idCOunt = count($id);
			 ...
			 for($i=0;$i<$idCOunt;$i++)
	           {
					 $sql = "UPDATE hj_product_details SET Qty='".$_POST["qty"][$i]."' WHERE ProductFullID='".$id[$i]."'";
 ...
 $product_info=mysqli_fetch_row(mysqli_query($con, "SELECT
    hj_product_info.ProductID
    , hj_product_info.ProductName
    , product_style.StyleName
    , hj_category_info.category_name
    , hj_product_info.Price
	, SUM(hj_product_details.Qty)
FROM

    hj_product_info
    INNER JOIN product_style 
        ON (LEFT(hj_product_info.ProductID,2) = product_style.StyleID)
	INNER JOIN hj_product_details
		ON(hj_product_info.SerialNo = hj_product_details.SerialNo)
    INNER JOIN hj_category_info 
        ON (SUBSTRING(hj_product_info.ProductID,3,2) = hj_category_info.category_id)
WHERE hj_product_info.ProductID='".$_REQUEST["ProductID"]."' AND hj_product_info.SerialNo='".$_REQUEST["serial"]."'"));
...
//     INNER JOIN product_category_info ON (product_sub_category.CategoryID = product_category_info.CategoryID) WHERE product_info.ProductID='".$_REQUEST["ProductID"]."'"));
	...
	$total_inserted_image=mysqli_fetch_row(mysqli_query($con, "SELECT COUNT(ImageID) FROM hj_product_image
	 WHERE ProductID='".$ProductID."' AND type='color'"));
?>
<HTML>
<HEAD>
		 ...
		 <tr>
		  ...
		  <?php
		   ...
		   $rs= mysqli_query($con, "SELECT
		     hj_product_details.ProductFullID
		    , color_list.ColorName
		    , color_list.ColorCode
		    , product_unit.UnitName
		    , hj_product_details.Qty
			FROM
			   hj_product_details
			    INNER JOIN color_list 
			        ON (SUBSTR(hj_product_details.ProductFullID,5,3) = color_list.ColorID)
			    INNER JOIN product_unit 
			        ON (RIGHT(hj_product_details.ProductFullID,2) = product_unit.UnitID)
			WHERE hj_product_details.SerialNo='".$_REQUEST["serial"]."'
			ORDER BY hj_product_details.ProductFullID");
		  $matching = 0;
//            $serial = $_REQUEST["serial"];
...
//		    </tr>";
        ...
        <table width="489" border="0" align="center" cellpadding="0" cellspacing="0">
            ...
            <tr>
                ...
                <?php
//                   $sl=0;
                   $rs=mysqli_query($con, "SELECT ImageID,ProductID,BigImage,MidImage,SmallImage,orderid FROM hj_product_image 
                   WHERE ProductID='".$ProductID."' AND SerialNo='".$_REQUEST['serial']."' ");
                   while($row=mysqli_fetch_row($rs))
                   {
//                    $sl++;
...
<!--                  <tr <?php // print $bgcolor; ?>>-->
                    <td align="center"> 
                        <img src="../productimage/hj/<?php print $row[4]; ?>" width="70" height="60" style="padding:2px;">	
