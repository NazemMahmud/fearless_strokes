<?php
 ...
 if(isset($_POST["btnSave"]))
 {
    $orderid=$_POST["orderid"];
	        ...
	        $qrs=mysqli_query($con, "SELECT ImageID,orderid FROM product_image WHERE ProductID='".$ProductID."' AND type='nocolor' ORDER BY orderid");
			$i=0;
					...
					if($qrow[1] != $orderid[$i])
						{
						 ...
						 WHERE ImageID='".$qrow[0]."' AND ProductID='".$ProductID."'");
						}	
					$i++;
				}
     $activity_update=mysqli_query($con, "UPDATE product_info SET UpdateDate='".$Date."', UpdateBy='".$_SESSION['UserID']."'
...
FROM
    ...
    INNER JOIN product_ssub_category 
        ON (RIGHT(product_info.ProductID,3) = product_ssub_category.SubSubCategoryID)
...
<table width="530" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
	...
	<span style="font-size:20px; font-weight:bold; color:#000000;"><?php print $product_info[3]; ?> </span>
	  <input type="hidden" name="ProductID" value="<?php print $_REQUEST["ProductID"]; ?>"  />  	  </td>
  </tr>

  <tr>
    <td width="105" height="32" align="center" valign="top"><strong>Product Name </strong></td>
    <td colspan="3" valign="top"><strong>: <?php print $product_info[1]; ?></strong></td>
  </tr>
 <tr>
    <td width="105" height="32" align="center" valign="top"><strong>Product Style </strong></td>
    <td colspan="3" valign="top"><strong>: <?php print $product_info[2]; ?></strong></td>
  </tr>
  <tr>
    <td height="31" align="center"><strong>Upload Image </strong></td>
	    ...
	    <?php
			$check_access=check_access($con, $_SESSION["UserID"],"MOD-04","add_option");
			 if($check_access=="yes")
			  {
			 ?>
	<input type="submit" name="btnAdd" class="btnsdt" id="sizebtn" value="Add Image" />	
	<?php } }?>
	<div id="valid_msg"/></td>
    </tr>
  ...
  <tr>
    ...
    <td width="136" align="center" bgcolor="#000000">&nbsp;</td>
  </tr>
  <?php
   $sl=0;
   $rs=mysqli_query($con, "SELECT ImageID,ProductID,SmallImage,orderid FROM product_image 
   WHERE ProductID='".$ProductID."' AND type='nocolor' ORDER BY orderid");
