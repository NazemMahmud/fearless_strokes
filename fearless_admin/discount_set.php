<?php
  include("connection.php");
	...
	WHERE CatID='".$_REQUEST["ID"]."' AND type='drange' AND  BelowRange=0"));
	
	$aboveCheck=mysqli_fetch_row(mysqli_query($con, "SELECT COUNT(CatID) FROM product_price_discount_range 
	WHERE CatID='".$_REQUEST["ID"]."' AND type='drange' AND  AboveRange=0"));
  ...
  if(isset($_POST["UpdateFromTO"]))
  {
	...
	WHERE BelowRange='0' AND CatID='".$_REQUEST["ID"]."' AND type='drange'");
	
	$update_above=mysqli_query($con, "UPDATE product_price_discount_range SET BelowRange='".$_POST["UpdateAbovePrice"]."'
	WHERE AboveRange='0' AND CatID='".$_REQUEST["ID"]."' AND type='drange'");
	
	print "<script>alert('Successfully Updated Below and Above Discount Range')</script>";
 ...
 if(isset($_POST["btnAdd"]))
 {
    ...
    VALUES('".$_POST["ID"]."','','".$_POST["BelowPrice"]."','drange')"); 
	 }
	 if($aboveCheck[0]==0)
	 {
	   $rs=mysqli_query($con, "INSERT INTO product_price_discount_range (CatID,BelowRange,AboveRange,type)
    VALUES('".$_POST["ID"]."','".$_POST["AbovePrice"]."','','drange')"); 
     
	 }
   $rs=mysqli_query($con, "INSERT INTO product_price_discount_range (CatID,BelowRange,AboveRange,type)
    VALUES('".$_POST["ID"]."','".$_POST["From"]."','".$_POST["To"]."','drange')"); 
   
   print "<script>alert('Successfully Inserted Discount Range')</script>";
 }
 if(isset($_POST["btnListUpdate"]))
 {
	 
	 $rs=mysqli_query($con, "SELECT id,BelowRange,AboveRange FROM product_price_discount_range WHERE type='drange'
...
WHERE product_ssub_category.SubSubCategoryID='".$_REQUEST["ID"]."'"));
     $title="Discount Range Settings for ".$title[0];
	}
...
<table width="530" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <?php
   ...
   ?>
  <tr>
    <td height="32" colspan="4" align="left" valign="top"><strong>Below  </strong><strong>or equal discount of</strong></td>
...
<?php } ?>
  <tr>
    <td height="28" colspan="4" align="left" valign="top"><strong>Discount Range greater </strong><strong>or equal of</strong></td>
...
%(e.g. only numeric data) </strong></td>
  ...
  <tr>
    <td height="28" colspan="4" align="left" valign="top"><strong>Discount Range less </strong><strong>or equal of</strong></td>
    <td height="28" colspan="3" align="left" valign="top"><strong>
    :
...
if($aboveCheck[0]==0)
{
  ?>
  <tr>
    <td height="28" colspan="4" align="left" valign="top"><strong>Above </strong><strong>or equal Discount of</strong></td>
    <td height="28" colspan="3" align="left" valign="top"><strong>:
...
<?php } ?>
  <tr>
    ...
    <td colspan="5" align="center"><input type="submit" name="btnAdd" class="btnsdt" value="Add Discount Range" /></td>
    <td align="center">&nbsp;</td>
  </tr>
  <?php } ?>
  <?php
  ...
  ?>
   <tr>
    <td height="28" colspan="4" align="left" valign="top"><strong>Below </strong><strong>or equal Discount of</strong></td>
    <td height="28" colspan="3" align="left" valign="top"><strong>:
...
(e.g. only numeric data) </strong></td>
 ...
 <tr>
    ...
    <td height="48" colspan="3" align="left" valign="middle"><input type="submit" name="UpdateFromTO" class="btnsdt" value="Update Below and Above Discount" /></td>
    </tr>
<?php }} ?>
  <tr>
    ...
    <td width="115" align="center" bgcolor="#000000"><span class="style4">Discount Range  </span></td>
  </tr>
  <?php
   $sl=0;
   $rs=mysqli_query($con, "SELECT id,BelowRange,AboveRange FROM product_price_discount_range WHERE type='drange'
 ...
 <?php } ?>
  ...
  <tr>
    <td height="37">&nbsp;</td>
    <td colspan="4" align="center"><input type="submit" name="btnListUpdate" class="btnsdt" value="Edit Discount Range" /></td>
    <td colspan="2"><input type="submit" name="btnDelete" class="btnsdt" value="Delete Discount Range" /></td>
    </tr>
  <?php } ?>
</table>
</form>
