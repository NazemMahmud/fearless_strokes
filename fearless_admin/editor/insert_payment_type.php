...
{	
			$PID=MakeID($con, "payment_type","PaymentID","PMTP-",7);
			$Date=MakeDate();
	        ...
	        $_POST['designdetail']=str_replace("'","&acute;",$_POST['designdetail']);
						...
						{	
						$file1 = $uploadfolder.$PID.$_FILES['photo']['name'];
						$bigimg=$PID.$_FILES['photo']['name']; 
						move_uploaded_file($_FILES['photo']['tmp_name'], $file1);
							  $sql="INSERT INTO payment_type (PaymentID,TypeName,APILink,description,ImageName,ActiveStatus) 
							  VALUES('".$PID."','".$_POST["title"]."','".$_POST["apilink"]."','".$_POST["designdetail"]."','".$bigimg."','Active')";
								$insert=mysqli_query($con, "$sql"); 
								if($insert)
								 {
								  print "<script>alert('Payment Method Successfully Inserted')</script>";
								 }
								else
								 if($insert)
								 {
						...
						{
						        $sql="INSERT INTO payment_type (PaymentID,TypeName,APILink,description,ImageName,ActiveStatus) 
							  VALUES('".$PID."','".$_POST["title"]."','".$_POST["apilink"]."','".$_POST["designdetail"]."','','Active')";
								$insert=mysqli_query($con, "$sql"); 
								if($insert)
								 {
								  print "<script>alert('Payment Method Successfully Inserted')</script>";
...
<table width="703" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td align="left" valign="top" id="subcat"><span class="input">
      <textarea name="apilink" cols="30" id="apilink"></textarea>
    </span> </td>
  </tr>
